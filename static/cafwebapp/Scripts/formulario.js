...
function cargaSendMail(){
    ...
    $('#c_mail').after("<span class='c_error' id='c_error_mail'>Ingresa un e-mail valido.</span>");
     //aplicamos color de borde si el se encontro algun error en el envio
    $('#contactform').css("border-color","#e74c3c");   
    sendMail = "false";
    }
    if (s_name.length == 0 ){
    $('#c_name').after( "<span class='c_error' id='c_error_name'>Ingresa tu nombre.</span>" );
    var sendMail = "false";
    }
    if (s_msg.length == 0 ){
    $('#c_msg').after( "<span class='c_error' id='c_error_msg'>Ingresa tu mensaje.</span>" );
    var sendMail = "false";
    }
    ...
    if(sendMail == "true"){
     ...
     $.ajax({
             ...
             //aplicamos color de borde si el envio es exitoso
                    $('#contactform').css("border-color","#88a707");
